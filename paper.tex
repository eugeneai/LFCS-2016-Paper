...
\institute{Irkutsk State University,\\ Irkutsk, Karl Marks str. 1, Russia\\
Institute of System Dynamics and Control Theory SB RAS,\\ Lermontov str. 134, Irkutsk, Russia\\
%\mailsa\\
%\mailsb\\
%\mailsc\\
...
%As a theoretical basis for our prover we have used the PCF calculus. Let's describe it in more details.
...
%Каждый узел дерева имеет вид или $\forall\bar{x}\colon A$ or $\exists\bar{x}\colon A$. Например, в формуле вида $\forall \bar{x}\colon A\left(G_1,\ldots,G_k\right)$ корневым узлом являюется узел $\forall \bar{x}\colon A$, а его дочерние узлы это $G_1,\ldots,G_k\right$.
...
After appropriate renaming some bound variables inside each subformula the expression $\bomega F$ will satisfy the well--formed PCF requirements. %in the canonical form requirements
